...
@torch.no_grad()
def bench_forward(module, x, iters=20, warmup=5):
    module.eval()
...
def bench_forward_backward(module, x, iters=10, warmup=3):
    module.train()
    for _ in range(warmup):
...
def main():
    ...
    print("running forward correctness")
    y_ref = slow(x)
    y_cuda = cuda_layer(x)
    print("forward max err:", (y_ref - y_cuda).abs().max().item())

    print("running backward correctness")
